...
async function LoadProduct(){
  ...
  products.forEach((product) => {
    if (product.Id == ItemValue) {
      ...
      const color = productElement.querySelector(".product__color");
      const desc = productElement.querySelector(".product__description");
      let photoSize= PhotoSize(product);;
      button.setAttribute("data-id", product.Id);
      button.addEventListener("click", function() {
        addProductToCart(product);
        let stringarray = JSON.stringify(selectedItems);
...
function PhotoSize(product){
  let width = window.innerWidth;
  let photoSize;
  if (width < 350) {
    photoSize = product.Images.PrimarySmall;
  } else if (width >= 350 && width <= 800) {
    photoSize = product.Images.PrimaryMedium;
  } else if (width > 800 && width <= 1000) {
    photoSize = product.Images.PrimaryLarge;
  } else { // width > 1000
    photoSize = product.Images.PrimaryExtraLarge;
  }
  return photoSize;
}